...
function history_30($uid, $offset) {
    $uid = (int)$uid;
    return do_cache("(history)_history_30:{$uid}:{$offset}", function() use($uid, $offset) {
        return query_mysql('select distinct story_id from history where user_id = '.$uid.' order by id desc limit 30 offset '.$offset);
    }, 86400);
}
...
function history_total_story_id_chapter_id($story_id, $chapter_id) {
    $story_id   = (int)$story_id;
    $chapter_id = (int)$chapter_id;
    return do_cache("(history)_history_total_story_id_chapter_id:{$story_id}:{$chapter_id}", function() use($story_id, $chapter_id) {
        $total = query_mysql("select count(id) as total from history where story_id = {$story_id} and chapter_id = {$chapter_id}", true);
